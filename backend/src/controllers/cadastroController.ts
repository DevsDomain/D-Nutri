...
class CadastroController {
  ...
  async getUserById(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const users = await pg.query(`SELECT "nomeUsuario", "email", "password" FROM "User" WHERE "idUsuario" = $1`, [idUsuario]);
...
async updateUser(req: Request, res: Response): Promise<Response> {
  try {
    ...
    return res.status(200).json({ message: "Usuário atualizado com sucesso!", updatedUser: updatedUser.rows[0] });
  } catch (error) {
